...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		# Initialize history
		self.history_manager = AgentHistoryManager(agent_id=self.id)
		...
		self.action_executor = AgentActionExecutor(
			...
			logger=self.logger,
			action_submitting_llm=self.llm,
			page_extraction_llm=self.settings.page_extraction_llm,
			extraction_schema=self.extraction_schema,
			sensitive_data=self.sensitive_data,
			available_file_paths=self.available_file_paths,
			check_stop_or_pause_callback=self._check_stop_or_pause,
			demo_mode_log_callback=self._demo_mode_log,
		)
	...
	@history.setter
	def history(self, value: AgentHistoryList) -> None:
	...
	async def _execute_actions(self) -> None:
		...
		result = await self.action_executor.execute_actions(self.state.last_model_output.action, self.state.n_steps)
	...
	async def rerun_history(
		...
		try:
			for i, history_item in enumerate(history.history):
				...
				if self.action_executor.is_redundant_retry_step(history_item, previous_item, previous_step_succeeded):
				...
				while retry_count < max_retries:
					try:
						result = await self.action_executor.execute_history_step(
							history_item, step_delay, ai_step_llm, wait_for_elements
						)
						...
						if (
							...
							and self.action_executor.is_menu_opener_step(previous_item)
						):
							...
							if self.action_executor.is_menu_item_element(curr_elem):
								self.logger.info(
									'🔄 Dropdown may have closed. Attempting to re-open by re-executing previous step...'
								)
								reopened = await self.action_executor.reexecute_menu_opener(previous_item, ai_step_llm)
								if reopened:
									menu_reopened = True
	...
	async def _execute_initial_actions(self) -> None:
		...
		if self.initial_actions and not self.state.follow_up_task:
			...
			result = await self.action_executor.execute_actions(self.initial_actions, 0)
	...
	def save_history(self, file_path: str | Path | None = None) -> None:
		...
		self.history_manager.save_to_file(file_path, sensitive_data=self.sensitive_data)
