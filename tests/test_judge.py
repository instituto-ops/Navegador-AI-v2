...
def test_construct_judge_messages_basic():
	...
	# SystemMessage.content can be a list or string. The .text property handles both.
	# However, we want to check for substrings which 'in' supports on strings.
	system_text = messages[0].text
	assert 'You are an expert judge' in system_text
	assert 'PRIMARY EVALUATION CRITERIA' in system_text

	# Check user message content
	user_content = messages[1].content
	# We expect a list here because screenshots are handled via list even if empty
	# But actually the implementation uses a list:
	# content_parts = [ContentPartTextParam(text=user_prompt)]
...
def test_construct_judge_messages_with_ground_truth():
	...
	assert 'GROUND TRUTH VALIDATION' in system_text
...
def test_construct_judge_messages_use_vision_true(mock_path, mock_file_open):
	...
	user_content = messages[1].content
	# Assert it's a list to help type checker
	assert isinstance(user_content, list)
	assert len(user_content) == 3  # 1 text + 2 images

	# Check images
	# We assert types for list elements to satisfy type checker
	img1 = user_content[1]
	img2 = user_content[2]
	assert isinstance(img1, ContentPartImageParam)
	assert isinstance(img2, ContentPartImageParam)

	# Verify image data format
	expected_b64 = base64.b64encode(b'fake_image_data').decode('utf-8')
	assert img1.image_url.url == f'data:image/png;base64,{expected_b64}'
