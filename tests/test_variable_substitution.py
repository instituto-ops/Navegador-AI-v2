...
def create_history_item(action_model: ActionModel) -> AgentHistory:
	return AgentHistory(
		model_output=AgentOutput(action=[action_model], evaluation_previous_goal='eval', memory='mem', next_goal='next'),
		result=[],
		state=BrowserStateHistory(
			url='http://example.com', title='Example', tabs=[], interacted_element=[], screenshot_path=None
		),
...
def test_substitute_variables_happy_path(agent):
	...
	detected_vars = {variable_name: DetectedVariable(name=variable_name, original_value=original_value)}

	with patch('browser_use.agent.variable_detector.detect_variables_in_history', return_value=detected_vars):
		new_history = agent._substitute_variables_in_history(history, {variable_name: new_value})
...
def test_substitute_variables_nested(agent):
	...
	action = ComplexAction(complex_op=NestedParams(details={'contact': {'email': original_email}}, tags=['tag1', original_email]))  # type: ignore

	history = AgentHistoryList(history=[create_history_item(action)])

	detected_vars = {variable_name: DetectedVariable(name=variable_name, original_value=original_email)}

	with patch('browser_use.agent.variable_detector.detect_variables_in_history', return_value=detected_vars):
		new_history = agent._substitute_variables_in_history(history, {variable_name: new_email})
...
def test_substitute_variables_no_match(agent):
	...
	detected_vars = {'some_var': DetectedVariable(name='some_var', original_value='Change Me')}

	with patch('browser_use.agent.variable_detector.detect_variables_in_history', return_value=detected_vars):
		new_history = agent._substitute_variables_in_history(history, {'some_var': 'New Value'})
...
def test_substitute_variables_unknown_var(agent, caplog):
	...
	with patch('browser_use.agent.variable_detector.detect_variables_in_history', return_value=detected_vars):
		with caplog.at_level(logging.WARNING):
			agent._substitute_variables_in_history(history, {'unknown_var': 'value'})
...
def test_substitute_multiple_variables(agent):
	...
	with patch('browser_use.agent.variable_detector.detect_variables_in_history', return_value=detected_vars):
		new_history = agent._substitute_variables_in_history(history, {'var1': 'New1', 'var2': 'New2'})
