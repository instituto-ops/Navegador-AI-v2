...
# Fallback to * if env var explicitly says so or for dev convenience if needed,
# but for security fix we respect the logic.
# If ALLOWED_ORIGINS is not set, it defaults to localhost:3000 which might break if user is using something else.
# But for security compliance, restricting it is the goal.
...
async def get_report(filename: str):
	...
	try:
		...
		# Should not happen if reports exists, but if it does, it might be path issue
