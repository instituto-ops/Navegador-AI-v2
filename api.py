...
async def get_or_create_browser():
	...
	async with shared_browser_lock:
		if shared_browser is None:
			...
			if not await Path(abs_profile_path).exists():
				await Path(abs_profile_path).mkdir(parents=True, exist_ok=True)
...
async def list_reports():
	try:
		...
		if not await reports_dir.exists():
			await reports_dir.mkdir(parents=True, exist_ok=True)

		# anyio doesn't have listdir directly in Path, use wrapper
		# anyio.Path.iterdir() returns async iterator
		files = []
		async for p in reports_dir.iterdir():
			files.append(p.name)
