...
async def get_or_create_browser():
    ...
    async with shared_browser_lock:
        if shared_browser is None:
            ...
            abs_profile_path = await asyncio.to_thread(os.path.abspath, os.path.join(os.getcwd(), 'browser_profile'))
            if not await asyncio.to_thread(os.path.exists, abs_profile_path):
                await asyncio.to_thread(os.makedirs, abs_profile_path)
...
async def list_reports():
    try:
        reports_dir = 'reports'
        if not await asyncio.to_thread(os.path.exists, reports_dir):
            await asyncio.to_thread(os.makedirs, reports_dir)
        reports = await asyncio.to_thread(os.listdir, reports_dir)
...
async def get_report(filename: str):
    try:
        ...
        reports_dir = await asyncio.to_thread(os.path.abspath, 'reports')
        path = await asyncio.to_thread(os.path.abspath, os.path.join(reports_dir, filename))
