...
async def get_or_create_browser():
	...
	async with shared_browser_lock:
		if shared_browser is None:
			...
			if not await aiofiles.os.path.exists(abs_profile_path):
				await aiofiles.os.makedirs(abs_profile_path)
...
async def list_reports():
	try:
		reports_dir = 'reports'
		if not await aiofiles.os.path.exists(reports_dir):
			await aiofiles.os.makedirs(reports_dir)
		return {'reports': await aiofiles.os.listdir(reports_dir)}
	except Exception as e:
		raise HTTPException(status_code=500, detail=str(e))
