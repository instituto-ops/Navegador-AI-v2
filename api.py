...
@app.post('/warmup')
async def warmup_browser(request: WarmupRequest):
	global current_agent_task
	print('\n--- [WARMUP] Iniciando aquecimento de perfil ---')
	...
	async def event_generator():
		...
		async def run_task():
			...
			try:
				browser = await get_or_create_browser()
				urls = ['https://www.google.com', 'https://www.bbc.com', 'https://www.wikipedia.org', 'https://www.globo.com']

				for i, url in enumerate(urls):
					elapsed = round(time.time() - start_time, 1)
					await queue.put(
						{'type': 'info', 'message': f'[WARMUP] Visiting {url} ({i + 1}/{len(urls)})...', 'elapsed': elapsed}
					)
				...
				await queue.put(
					{
						'type': 'done',
						'message': 'Browser profile warmed up successfully!',
						'total_time': round(time.time() - start_time, 1),
					}
				)

			except asyncio.CancelledError:
				await queue.put({'type': 'info', 'message': 'Aquecimento interrompido.'})
			except Exception as e:
				await queue.put({'type': 'error', 'message': f'Erro fatal: {str(e)}'})
			finally:
				logger.removeHandler(handler)
